...
返回值类型：方法执行结果的数据类型
方法名：方法的名称
参数列表：参数类型可以是基本类型，也可以是引用类型；参数名即变量名
方法体语句：完成特定功能的代码
return: 用于结束方法
返回值：方法执行之后的结果，若方法没有返回值，则它的返回值类型为void，比如main方法
...
- 方法必须定义在类中
- 方法之间属于平级关系，不能嵌套定义
- 方法没有返回时，也要有返回值的类型 void
- 方法返回值的类型为void时，可以省略return语句
...
形式参数：在定义方法时需要明确方法的参数形式，比如参数的类型和个数
实际参数：调用方法时传递给方法的数据，必须符合形式参数的个数和类型


### (5) 方法重载

同一个类中的多个方法，它们的方法名相同，参数列表不同。方法重载与返回值的类型无关。
...
当实现的功能相同，但具体的实现方式不同时，可以通过定义名称相同，参数（条件）不同的方法，来更好的识别和管理类中的方法。
...
java程序的内存分配
- 方法区（Method Area）: 存储可运行的class文件，包含方法、静态成员、常量等。
- 栈（stack）: 方法运行时使用的内存，先进后出
